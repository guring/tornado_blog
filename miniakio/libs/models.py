...
class PostMixin(object):
    ...
    def get_posts_by_tag(self, tag):
        sql = """SELECT p.id, p.slug, p.title, p.published FROM posts AS p
    ...
    def get_posts_by_category(self, category):
        sql = """SELECT id,slug, title, published FROM posts
                 WHERE category = ?
    ...
    def get_count_posts(self, count=None):
        if count:
            ...
            posts = self.db.query("SELECT id,slug,title,published FROM posts ORDER BY published DESC;")
        return posts
    ...
    def get_next_prev_post(self, published):
        next_post = self.db.get(
            "SELECT id,slug,title FROM posts WHERE published > ? ORDER BY published ASC LIMIT 1;",
            published)

        prev_post = self.db.get(
            "SELECT id,slug,title FROM posts WHERE published < ? ORDER BY published DESC LIMIT 1;",
            published)
